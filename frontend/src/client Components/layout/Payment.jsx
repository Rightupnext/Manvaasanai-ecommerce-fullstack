...
const Payment = ({totalAmount}) => {
  ...
  const loadRazorpay = async () => {
    dispatch(createOrder(totalAmount)).then((action) => {
      if (action.meta.requestStatus === "fulfilled") {
        const { id, amount, currency } = action.payload;
        ...
        const options = {
          ...
          order_id: id,
          handler: (response) => {
            dispatch(verifyPayment(response)).then((verifyAction) => {
  ...
  return (
    ...
    <button
    onClick={loadRazorpay} disabled={loading}
    type="button"
