...
const Payment = ({ totalAmount, quantities, setCurrentStep }) => {
  ...
  const loadPhonepe = async () => {
    const shippingAddress = JSON.parse(
      localStorage.getItem("manvaasanai_Shipping_Address")
    );
  
    try {
      // 🟢 Step 1: Dispatch createOrder to Backend
      const action = await dispatch(
        createOrder({
          amount: totalAmount,
          mobileNumber: Number(shippingAddress?.phoneNumber),
          name: shippingAddress?.fullName,
        })
      );
  
      if (action.meta.requestStatus === "fulfilled") {
        const { success, paymentUrl, orderId } = action.payload;
  
        if (success && paymentUrl) {
          // 🟢 Step 2: Redirect to PhonePe Payment Page
          window.location.href = paymentUrl;
  
          // Wait for the payment process to complete, then verify it
          setTimeout(async () => {
            try {
              // 🟢 Step 3: Verify Payment after Redirection
              const verifyAction = await dispatch(verifyPayment({ orderId }));
  
              if (verifyAction.payload.success) {
                console.log("✅ Payment Verified Successfully");
  
                // 🟢 Step 4: Place Order after Payment Confirmation
                await dispatch(
                  placeOrder({
                    products: products,
                    amount: totalAmount,
                    shippingAddress,
                    paymentId: verifyAction.payload.transactionId,
                  })
                )
                  .then((res) => res.json())
                  .then((data) => {
          ...
          }, 5000); // Delay for redirection to complete
        } else {
