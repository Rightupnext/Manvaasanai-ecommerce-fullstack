...
import { getProductsByIdReviews } from "../../store/reducers/productReducers";
...
function Reviews() {
  ...
  const totalreviewCount = reviews.totalReviewsCount?.totalReviews;
  const averageRating = reviews.totalReviewsCount?.averageRating;
  const totalReviews = reviews.totalReviewsCount?.totalReviews || 1; // Avoid division by zero

  const getPercentage = (count) =>
    ((count / totalReviews) * 100).toFixed(2) + "%";

  const ratingCount1 = reviews?.ratingBreakdown?.["1"] || 0;
  const ratingCount2 = reviews?.ratingBreakdown?.["2"] || 0;
  const ratingCount3 = reviews?.ratingBreakdown?.["3"] || 0;
  const ratingCount4 = reviews?.ratingBreakdown?.["4"] || 0;
  const ratingCount5 = reviews?.ratingBreakdown?.["5"] || 0;

  const ratings = [
    { label: "5", count: ratingCount5 },
    { label: "4", count: ratingCount4 },
    { label: "3", count: ratingCount3 },
    { label: "2", count: ratingCount2 },
    { label: "1", count: ratingCount1 },
  ];
        ...
        <div className="mx-auto max-w-screen-xl px-4 2xl:px-0">
          ...
          <div className="mt-6 divide-y divide-gray-200 dark:divide-gray-700">
            {reviews && reviews.reviews && reviews.reviews.length > 0 ? (
              ...
              <p>No reviews available.</p> // This will display if there are no reviews
            )}
          </div>
